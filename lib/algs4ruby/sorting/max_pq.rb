module Algs4Ruby
  # MaxPQ
  #
  # The MaxPQ class represents a priority queue of generic keys. It supports the
  # usual insert and delete-the-maximum operations as well as methods for
  # peeking at the maximum key, testing if the priority queue is empty, and
  # iterating over the keys.
  #
  # This implementation uses a binary heap. The {#insert} and {#delete_max}
  # operations take amortized logarithmic time. The {#max}, {#size}, and
  # {#empty?} operations all take constant time. Initializing a new MaxPQ takes
  ...
  # empty, it removes the maximum element from the pq and prints it to
  ...
  class MaxPQ
    ...
    # @overload intialize
    #   Initializes a MaxPQ which compares items using the less than operator
    #   (#<).
    # @overload intialize(&comparator)
    #   Initializes a MaxPQ which compares items using the comparator block.
    #   @yieldparam a [Object]
    #   @yieldparam b [Object]
    ...
    #   Initializes a MaxPQ from the enumerable of keys. Compares items using
    ...
    # Returns the largest key in the pq.
    # @return [Object]
    ...
    # Removes the largest key from the pq and returns it.
    # @return [Object]
    # @raise [PriorityQueueEmptyError] if the pq is empty.
    def delete_max
      fail PriorityQueueEmptyError if empty?
    ...
    def each
      ...
      copy = MaxPQ.new(@size, &@comparator)
      (1..@size).each { |i| copy.insert(@pq[i]) }
      yield copy.delete_max until copy.empty?
    ...
    #   Initializes a MaxPQ which compares items using the given comparator
    ...
    # Is key at a less than key at b?
  ...
  if __FILE__ == $PROGRAM_NAME
    ...
    while l = gets
      l.chomp!
      if l == '-'
        puts pq.delete_max unless pq.empty?
      else
        pq.insert(l)
      end
    end
...
###############################################################################
# The code in this file is based on the java code in MaxPQ from alg4.jar
...
# website as well.
#
# max_pq.rb is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# max_pq.rb is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
# You should have received a copy of the GNU General Public License
# along with max_pq.rb.  If not, see http://www.gnu.org/licenses.
################################################################################
